...
void ping(const std::string& addr, uint8_t iters) {
    ...
    cout << "Pinging" << addr << "with " << dataSize << " of data:" << std::endl;
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        std::cout << "Reply from: " << s_ip << ": bytes=" << reply->DataSize << " time=" << reply->RoundTripTime << "ms TTL=" << (int) reply->Options.Ttl << std::endl;
    }
